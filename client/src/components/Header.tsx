...
const Li: React.FC<{ children: React.ReactNode; onClick?: () => void }> = ({
  ...
  return (
    ...
    >
      ...
      <div className='absolute inset-0 border-b-2 border-primaryYellow w-0 group-hover:w-full transition-all duration-300 ease-in-out'></div>
    </li>
  );
};
...
const Header: React.FC<HeaderProps> = ({ homeRef, servicesRef, contactRef }) => {
    ...
    <header className='w-full py-4'>
      <nav className=' flex items-center justify-between'>
        ...
        >
          ...
          <div className='absolute inset-0 z-1  bg-primaryYellow w-0 group-hover:w-full transition-all duration-300 ease-in-out'></div>
        </button>
      </nav>
    </header>
  );
